function [OUTEEG, com] = pop_icablinkmetrics(INEEG)
    ...
    if isobject(INEEG) % eegobj
        ...
        if isempty(INEEG)
            ...
            if isempty(INEEG.data) | isempty(INEEG.icaweights)
                disp('Error in pop_icablinkmetrics(): This function cannot run on an empty EEG dataset or a dataset that can not had ICA weights computed.')
                beep
            else
                ...
                g2 = [0.05 0.475 0.475];
                s1 = [1];
                geometry = { s1 g2 g2 g2 s1 s1 g2 g2 g2 s1 s1 s1};
                uilist = { ...
                      ...
                      { } ...
                      ...
                      { 'Style', 'text', 'string', 'If you are not sure what to do, just click OK. The function will search for a channel'} ...
                      ...
                      { 'Style', 'text', 'string', 'which manifests the eye blink and determine when the eyeblinks occur.'} ...
                      ...
                  };
                  ...
                  if ~isempty(structout)
                      ...
                      if (structout.EEGDATA > 1)
                          ...
                          if (structout.Correlation == structout.Convolution) & (structout.Correlation == structout.PercentReduction)
                            com = sprintf('Equivalent Code:\n%s.icaquant = icablinkmetrics(%s,''ArtifactChannel'',EEG.data(find(strcmp({EEG.chanlocs.labels},''%s'')),:),''Alpha'',%s,''VisualizeData'',''True'');', inputname(1),inputname(1),INEEG.chanlocs((structout.EEGDATA-1)).labels,mat2str(structout.Correlation));
                          else
                            com = sprintf('Equivalent Code:\n%s.icaquant = icablinkmetrics(%s,''ArtifactChannel'',EEG.data(find(strcmp({EEG.chanlocs.labels},''%s'')),:),''MetricThresholds'',%s,''VisualizeData'',''True'');', inputname(1),inputname(1),INEEG.chanlocs((structout.EEGDATA-1)).labels,mat2str([structout.Correlation,structout.Convolution,structout.PercentReduction]));
                          end
                          disp(com)
                      ...
                      elseif (structout.EEGSKIP > 1)
                              ...
                              if (structout.Correlation == structout.Convolution) & (structout.Correlation == structout.PercentReduction)
                                com = sprintf('Equivalent Code:\n%s.icaquant = icablinkmetrics(%s,''ArtifactChannel'',EEG.skipchannels.data(%d,:),''Alpha'',%s,''VisualizeData'',''True'');', inputname(1), inputname(1), (structout.EEGSKIP-1),mat2str(structout.Correlation));
                              else
                                com = sprintf('Equivalent Code:\n%s.icaquant = icablinkmetrics(%s,''ArtifactChannel'',EEG.skipchannels.data(%d,:),''MetricThresholds'',%s,''VisualizeData'',''True'');', inputname(1), inputname(1), (structout.EEGSKIP-1),mat2str([structout.Correlation,structout.Convolution,structout.PercentReduction]));
                              end
                              disp(com)
                              disp('ICA Metrics are located in:    EEG.icaquant.metrics')
                              if (OUTEEG.icaquant.identifiedcomponents ~= 0)
                          ...
                          OUTEEG.icaquant = icablinkmetrics(INEEG, 'MetricThresholds',[structout.Correlation,structout.Convolution,structout.PercentReduction],'VisualizeData', 'True');
                          if (structout.Correlation == structout.Convolution) & (structout.Correlation == structout.PercentReduction)
                              com = sprintf('Equivalent Code:\n%s.icaquant = icablinkmetrics(%s,''Alpha'',%s,''VisualizeData'',''True'');', inputname(1), inputname(1), mat2str(structout.Correlation));
                          else
                              com = sprintf('Equivalent Code:\n%s.icaquant = icablinkmetrics(%s,''MetricThresholds'',%s,''VisualizeData'',''True'');', inputname(1), inputname(1), mat2str([structout.Correlation,structout.Convolution,structout.PercentReduction]));
                          end
                          disp(com)
                          disp('ICA Metrics are located in:    EEG.icaquant.metrics')
                          if (OUTEEG.icaquant.identifiedcomponents ~= 0)
