...
%       k     'VoltageThreshold' - [ 'True' [default]| 'False' ] Boolean parameter to use statistically improbably voltage deflections if the template matching procedure fails.
%
%   Example Code:
%
...
%   revised 5-30-17 to enable the function to self-determine the artifact channel if one is not specified
%   revised 8-2-19 to use icachansind to select those channels which were included in ICA computation (non destructive)
    ...
    try, r.VoltageThreshold; catch, r.VoltageThreshold = 'True'; end
    ...
    if isempty(INEEG.icaact)
        INEEG.data = INEEG.data([INEEG.icachansind],:);
    ...
    try
        ...
        try
            % Search for latencies and channels
            [iBlinkLatencies, iBlinkChannel] = eyeblinklatencies4ica(INEEG, 'Threshold', 0.96);
            if ~isempty(iBlinkChannel)
                r.ArtifactChannel = INEEG.data(iBlinkChannel,:);
            else
                error('Error at icablinkmetrics(). Missing information! Please input Artifact channel information.'); 
            end
            if ~isempty(iBlinkLatencies)
                r.ArtifactLatencies = iBlinkLatencies;
            end
        catch
            error('Error at icablinkmetrics(). Missing information! Please input Artifact channel information.'); 
        end
    ...
    try
        ...
        if (numel(r.ArtifactLatencies) < r.MinimumArtifacts)
            ...
            if strcmpi(r.VoltageThreshold, 'True')
                tDat = fastsmooth(r.ArtifactChannel, floor(INEEG.srate/15), 2, 1);% Smooth the artifact channel to remove high frequency noise
                spantime = floor((size(tDat,2)/INEEG.srate)/120);
                [pksp, locsp] = findpeaks(tDat); % obtain peaks
                pksp = abs(trimzscore(pksp,'trimMin', spantime, 'trimMax',spantime)); % Z score and rectify data
                [pksn, locsn] = findpeaks(tDat*-1); % obtain inverse peaks
                pksn = abs(trimzscore(pksn,'trimMin', spantime, 'trimMax',spantime)); % Z score and rectify data
                r.ArtifactLatencies = sort(horzcat(locsp(find(pksp>2.2)),locsn(find(pksn>2.2)))); % toss out any peaks that are small                
            end
        end
    end
    if (numel(r.ArtifactLatencies) < r.MinimumArtifacts)
    ...
    fprintf(1, 'icablinkmetrics(): Removing Components (with replacement) - |')
    
    % Setup Controls for Progress Bar
    WinStart = 1;
    WinStop = size(INEEG.icaweights,1);
    nSteps = 25;
    step = 1;
    ...
    for compindex = 1:size(INEEG.icaweights,1) % For each ICA Component
        ...
        [step, strLength] = commandwaitbar(compindex, WinStop, step, nSteps, strLength); % progress bar update
    end
