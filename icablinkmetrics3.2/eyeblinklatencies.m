...
%   Author: Matthew B. Pontifex, Health Behaviors and Cognition Laboratory, Michigan State University, July 28, 2014
    ...
    try
        ...
        tDatxcorr = xcorr(double(tDat),double(iBlink)); % Obtain cross correlation
        ...
        for i = 1:numel(tDatxcorrZsel)
            ...
            [step, strLength] = commandwaitbar(i, WinStop, step, nSteps, strLength);
        end
        % Closeout bar
    ...
    if isempty(iBlinkLatencies)
        ...
        step = 1;
        % Start Loop
        fprintf(1, '\neyeblinklatencies(): Alternative Eyeblink Latency Search - |')
        strLength = fprintf(1, [repmat(' ', 1, nSteps-step) '|   0%%']);
        % Setup Controls
        WinStart = 1;
        WinStop = (size(tDat,2)-size(iBlink,2));
        tic
        pointindexStart = 1;
        ...
        while pointindexStart < (size(tDat,2)-size(iBlink,2))
            ...
            [step, strLength] = commandwaitbar(pointindexStart, WinStop, step, nSteps, strLength);
        end
        % Closeout bar
        [step, strLength] = commandwaitbar(WinStop, WinStop, step, nSteps, strLength);
        fprintf(1, '\n')
